"""Benchmark harness for the LARGE codebase (15 files, 11 bugs).

Runs both agents N times and compares results on apps/target_app_large/.

Outputs:
- results/benchmark_large_YYYY-MM-DD.json (raw data)
...
def run_benchmark_large() -> None:
    ...
    console.print(f"\n[bold cyan]Context Engineering Benchmark — Large App (15 files)[/bold cyan]")
    ...
    try:
        reset_target_app_large(silent=True)
    ...
    for i in range(n_runs):
        console.rule(f"Run {i + 1}/{n_runs} — Traditional Agent (Large)")
        reset_target_app_large(silent=True)
    ...
    for i in range(n_runs):
        console.rule(f"Run {i + 1}/{n_runs} — Context-Engineered Agent (Large)")
        reset_target_app_large(silent=True)
        from .logger import session_tokens
        session_tokens.reset()
        t0 = time.monotonic()
        result = run_context_agent(config, target_files=TARGET_FILES_LARGE, test_dir=TEST_DIR_LARGE)
        result["elapsed_s"] = round(time.monotonic() - t0, 1)
        result["cost_usd"] = _estimate_cost(result.get("total_tokens", 0), config.provider)
    ...
    console.print()
    table = Table(title="Benchmark Results — Large App", show_header=True, header_style="bold magenta")
    ...
    out_path = results_dir / f"benchmark_large_{date_str}.json"
